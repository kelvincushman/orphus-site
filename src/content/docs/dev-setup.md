...
CI runs static checks, the root unit/integration suites, the coding-agent suite, and release-archive smoke tests as concurrent jobs behind a fail-closed result gate, on Linux and Windows. It builds `@orphus/natives` explicitly before the suites. See [docs/ci.md](/docs/ci) and `.github/workflows/test.yml`.
